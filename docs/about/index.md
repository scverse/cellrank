...
- {mod}`~cellrank.kernels` that take multi-view single cell input data and estimate a matrix of cell-cell
  transition probabilities $T$. Row $i$ in matrix $T$ contains the transition probabilities from cell
  $i$ towards putative descendants. Therefore, all entries in the matrix are between 0 and 1, and rows sum to one.
- {mod}`~cellrank.estimators` that take a cell-cell transition matrix $T$ computed using any kernel and
...
All CellRank kernels yield sparse transition matrices $T$. Further, the {class}`~cellrank.estimators.GPCCA`
...
inversion problem into a set of linear systems, which we solve in parallel using the sparsity-optimized [GMRES] algorithm, implemented
efficiently in [PETSc]. We use similar tricks to infer macrostates of cellular dyanmics via sparsity-optimized partial real Schur
decompositions (implemented under the hood via [pyGPCCA] and [SLEPc]).
